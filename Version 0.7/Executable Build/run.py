...
class WuxiaCoScraper(object):
    ...
    def __init__(self, window):
        self.window = window
        self.window.title("m.Wuxiaworld.co Scraper")
        self.window.configure(background="black")

        # Labels
        Label(self.window, text="Novel Link", bg="black", fg="white", font="none 16").grid(row=0, column=0, sticky=W)
        Label(self.window, text="Cover Page [optional]: ", bg="black", fg="white", font="none 10").grid(row=2, column=0,
                                                                                                        sticky=W)
        # Entries
        self.eNovel = Entry(self.window, width=75, bg="white")
        self.eNovel.grid(row=0, column=1, sticky=W)
...
def okButtonClick():
    if tkvar.get() == "NovelPlanet":
        ...
        def mwcallback():
            mwwindow = Tk()
            mwGUI = WuxiaCoScraper(mwwindow)
            mwwindow.mainloop()
        t = threading.Thread(target=mwcallback)
        t.daemon = True
        t.start()
