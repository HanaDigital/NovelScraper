...
class LaunchPanel(wx.Panel):
    ...
    def nodejsUpdate(self):
        ...
        downloadMAC = Button(popup, text="Download", fg="blue", command=self.installNodejsMAC)
        # okButton = Button(popup, text="OK", command=popup.destroy)

        label.pack()
        downloadMAC.pack(side=BOTTOM)

        popup.call('wm', 'attributes', '.', '-topmost', '1')
        popup.mainloop()

    # Open the link to the nedejs 32bit on a browser
    def installNodejsMAC(self):
        webbrowser.open_new(r"https://nodejs.org/dist/v10.16.0/node-v10.16.0.pkg")
        os._exit(1)
